...
public class MazeBuilder : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (GameManager.NetworkMode == true)//true is host
        {
        ...
        else
        {
            ...
            GameEvents.current.OnSendSeedTrigger += SetSeed;
            GameEvents.current.ReadyToRecieveSeedTrigger();
        }

    }
    #endregion

    IEnumerator WaitForSeedValueUpdate()
    {
    ...
    private void SetSeed(int seed)
    {
        ...
        if (seed == 0)
        {
            ...
            StartCoroutine(WaitForSeedValueUpdate());
    ...
    public int BuildMaze()
    {
        ...
        GameEvents.current.SendSeedTrigger(seed);


        GameEvents.current.OnPlayerPositionTrigger += OnPlayerPositionChanged;
        GameEvents.current.OnPlayerReadyForPositionTrigger += OnPlayerPositionAvailable;
        GameEvents.current.OnTileDestroyedTrigger += OnTileDestroyed;

        return seed;
    }

    //private Dictionary<int, Vector3> PlayerPositions;
    private List<PlayerData> PlayerPositions = new();

    private class PlayerData
    {
        readonly int id;
        Vector3 position;
        public PlayerData(int playerID, Vector3 playerPosition)
        {
            id = playerID;
    ...
    private void Update()
    {
        ...
        {
            BuildMazeChunk(PlayerPositions[i].Position);
        }
    ...
    private void OnPlayerPositionAvailable(int playerID)
    {
        ...
        //add player to list of player data
        PlayerPositions.Add(new PlayerData(playerID, playerStartPos));
    ...
    {
        foreach (PlayerData player in PlayerPositions)
        {
    ...
    private class MazeNode
    {
        ...
        //used for path nodes
        private static Dictionary<int[], MazeNode> pathNodes = new();
    ...
    /// <summary>
    /// Gets the maze size from the game manager and checks if its a valid maze size; defaults to 10x10
    /// </summary>
